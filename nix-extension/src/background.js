...
//
// Nix used to drive a single shared tab and navigate it between conversations.
...
const storageReady = new Promise((resolve) => {
  try {
    chrome.storage.local.get('nix_conv_tabs', (d) => {
      const stored = d && d.nix_conv_tabs;
...
function persistConvTabs() {
  chrome.storage.local.set({ nix_conv_tabs: convTabs });
}
  ...
  ws.onmessage = (event) => {
    try {
      ...
      if (msg.type === 'reload_extension') {
        ...
        chrome.storage.local.set({ nix_refresh_owned_tabs: true }, () => {
      ...
      console.error('[Nix] Failed to parse WS message:', e);
    }
  };
...
// NOTE: every tabs.create/update below uses `active: false` ON PURPOSE. Nix drives
...
chrome.runtime.onConnect.addListener((port) => {
  if (port.name !== 'nix-content') return;
  ...
  function forward(msg) {
    ...
    try {
      ...
      console.error('[Nix] ws.send failed:', e);
    }
    if (
      delivered &&
  ...
  storageReady.then(() => {
    ...
    // Only tabs Nix opened for a conversation take part. The content script is
...
self.addEventListener('unhandledrejection', (e) => {
  console.warn('[Nix] unhandled rejection:', e.reason);
  e.preventDefault();
});
...
try {
  chrome.alarms.create('nix-keepalive', { periodInMinutes: 0.5 });
  chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === 'nix-keepalive') ensureConnected();
  });
} catch (_) {}
...
storageReady.then(() => {
  chrome.storage.local.get('nix_refresh_owned_tabs', (data) => {
    if (!data || !data.nix_refresh_owned_tabs) {
      setTimeout(() => { ensureContentScript().catch(() => {}); }, 300);
      return;
    }

    chrome.storage.local.remove('nix_refresh_owned_tabs');
