...
globalThis.__nixContentLoaded = true;

let backgroundPort = null;
let sending = false;
// Start inert. The background explicitly resumes only tabs that belong to a Nix
...
window.addEventListener('message', (e) => {
  ...
  if (d.__nix_models === true && Array.isArray(d.models)) {
    postToBg({ type: 'models', models: d.models });
    return;
  }
  if (d.__nix_net_request === true) {
  ...
  if (d.__nix_net_metric === true && d.metric) {
    postToBg({
  ...
  if (d.__nix_net_debug === true) { wlog(`sse sample: ${d.sample}`); return; }
  if (d.__nix_net !== true) return;
  if (!currentSend) {
    console.warn('[Nix] net response dropped — no currentSend (was port reset?)');
    return;
  }
...
function connectBackground() {
  ...
  try {
    ...
    console.warn('[Nix] extension context lost — this page needs a refresh');
    showReloadWarning();
    return;
  }

  try {
    backgroundPort = chrome.runtime.connect({ name: 'nix-content' });
  } catch (e) {
    reconnectAttempts++;
    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
      console.warn('[Nix] max reconnect attempts reached — extension may need a page refresh');
      showReloadWarning();
      return;
    }
    const delay = Math.min(1000 * Math.pow(2, reconnectAttempts), 30000);
    console.warn('[Nix] connect failed (attempt', reconnectAttempts, '):', e.message, '— retry in', delay, 'ms');
  ...
  backgroundPort.onDisconnect.addListener(() => {
    ...
    if (chrome.runtime.lastError) {
      console.warn('[Nix] port disconnected:', chrome.runtime.lastError.message);
    }
    sending = false;
  ...
  for (const msg of queue) {
    try { backgroundPort.postMessage(msg); } catch (e) {
      console.error('[Nix] flush failed:', e);
    }
  }
...
function requestModelsFromPage() {
  const ask = () => {
    if (standby) return;
    try { window.postMessage({ __nix_request_models: true }, '*'); } catch (_) {}
  };
  ask();
...
function handleMessage(msg) {
  ...
  // This tab isn't the one Nix drives — go quiet so we don't report our URL or
  // act on anything meant for the owned tab.
  if (msg.type === 'standby') {
    standby = true;
    stopOwnedPageFeatures();
    console.log('[Nix] standing down — this is not the Nix tab');
    return;
  }
  ...
  if (msg.type === 'resume') {
    if (standby) {
      ...
      console.log('[Nix] resumed — this tab is Nix-owned');
    }
    return;
  }
  if (standby) {
    ...
    startOwnedPageFeatures();
    console.log('[Nix] resumed from an owned page command');
  }
  switch (msg.type) {
    case 'recover_turn':
    ...
    case 'switch_model':
      ...
      console.log('[Nix] switch_model recv → model:', msg.model || '(keep)', '| intelligence:', msg.intelligence || '(keep)', sending ? '(BUSY)' : '');
      if (sending) {
...
function scrapeModelMenu(force = false) {
  try {
    ...
    console.log('[Nix] model menu →', currentModel || '(?)',
...
async function seedModelMenu() {
  ...
  try {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (lastMenuSig) {
        seededMenu = true;
        console.log('[Nix] seed skipped — menu already captured');
        return;
      }
    ...
    if (!trigger) { console.warn('[Nix] seed: model button not found after 20s'); return; }
    if (standby) return;
    seededMenu = true;
    console.log('[Nix] seeding model menu via', (trigger.textContent || '').trim().slice(0, 24));
    ...
    if (modelRow) {
      ...
      console.warn('[Nix] seed: model row not found (menu may not have opened)');
    }
    document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
    await sleep(150);
    document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  } catch (e) {
    console.warn('[Nix] seed failed:', e);
  } finally {
    seedingMenu = false;
  }
}
...
// menu state before Nix is allowed to submit a message.
...
async function selectModelViaDom(model, intelligence) {
  ...
  try {
    ...
    console.log('[Nix] switch → model:', model || '(keep)', '| intelligence:', intelligence || '(keep)');
    ...
    if (model) {
      ...
      if (!currentModel || normModelText(currentModel) !== normModelText(model)) {
        ...
        realClick(target);
        console.log('[Nix] switch: clicked model', model);
    ...
    if (intelligence) {
      ...
      if (el) {
        ...
        console.log('[Nix] switch: confirmed intelligence', intelligence);
      } else {
        const state = scrapeModelMenu(true);
    ...
    console.warn('[Nix] selectModelViaDom failed:', e);
...
function conversationId(url) {
  return globalThis.NixProtocolCore?.conversationId(url) || null;
}
...
// while Nix is driving a turn in this tab.
function armNetHooks(on, turnId) {
  if (!on) {
    try {
      window.postMessage({ __nix_arm: true, armed: false, turnId: turnId || '' }, '*');
    } catch (_) {}
  ...
  try {
    window.postMessage({ __nix_arm: true, armed: true, turnId: turnId || '' }, '*');
  } catch (_) {
...
async function sendMessageToChatGPT(text, msgId, model, intelligence, expectUrl) {
  ...
  try {
    ...
    console.log('[Nix] waiting for network response');
...
function isCompleteNixResponse(value) {
  const text = String(value || '').trim();
  const opening = /\{nix:(note|run|write|patch|done)\b[^}]*\}/g;
  let cursor = 0;
  let found = false;
  let match = null;
  while ((match = opening.exec(text)) !== null) {
    if (text.slice(cursor, match.index).trim()) return false;
    const close = `{/nix:${match[1]}}`;
...
function completedResponseAfterPrompt(prompt) {
  ...
  for (let i = userIndex + 1; i < messages.length; i++) {
    ...
    if (isCompleteNixResponse(text)) return text;
  }
  return '';
}
...
function postToBg(msg) {
  if (backgroundPort) {
    try {
      ...
      console.error('[Nix] postToBg failed:', e);
...
function showReloadWarning() {
  const existing = document.getElementById('__nix_reload_warning');
  if (existing) return;
  const banner = document.createElement('div');
  banner.id = '__nix_reload_warning';
  ...
  banner.textContent = '⚠ Nix extension was reloaded — click to refresh this page';
  banner.onclick = () => location.reload();
  document.body.prepend(banner);
  // Also try to auto-reload after 30s
  setTimeout(() => { if (document.getElementById('__nix_reload_warning')) location.reload(); }, 30000);
}
