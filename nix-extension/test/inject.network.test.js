...
test('a fetch handoff continues on the socket and completes from network text', () => {
  ...
  const message = {
    message: {
      ...
      content: { content_type: 'text', parts: ['{nix:note}Reading'] },
    },
  }
  const append = {
    o: 'append',
  ...
  const responsePosts = harness.messages.filter((message) => message.__nix_net)
  assert.equal(responsePosts.at(-1).done, true)
  assert.equal(responsePosts.at(-1).source, 'network')
  assert.equal(
    responsePosts.at(-1).text,
    '{nix:note}Reading repository structure{/nix:note}\n{nix:run}ls -la{/nix:run}',
  )

  const metric = harness.messages.find((message) => message.__nix_net_metric).metric
...
test('arming the page hook acknowledges the exact turn before submission', () => {
  ...
  harness.listeners.get('message')({
    ...
    data: { __nix_arm: true, armed: true, turnId: 'turn-ready' },
  })

  const ready = harness.messages.find((message) => message.__nix_net_arm_ready)
  assert.equal(ready.__nix_net_arm_ready, true)
...
test('a message boundary cannot truncate a turn that continues streaming', () => {
  ...
  assert.equal(sink.done, true)
  const response = harness.messages.filter((message) => message.__nix_net).at(-1)
  assert.equal(
    response.text,
