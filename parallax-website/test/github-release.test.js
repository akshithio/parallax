...
test('prefers the universal macOS image and identifies the extension package', () => {
  assert.equal(chooseAsset(assets, 'macos').name, 'Parallax-0.1.0-universal.dmg');
  assert.equal(chooseAsset(assets, 'extension').name, 'Parallax-Extension-0.1.0.zip');
...
test('normalizes release metadata for the website', () => {
  const release = normalizeRelease({
    ...
    html_url: 'https://github.com/akshithio/parallax/releases/tag/v0.1.0',
    assets,
  });
...
test('fetches the public latest-release endpoint with explicit GitHub headers', async () => {
  ...
  assert.match(request.url, /repos\/akshithio\/parallax\/releases\/latest$/);
