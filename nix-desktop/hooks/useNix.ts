...
declare global {
  interface Window { nix: any; __nix_project_cwd__?: string }
}
...
function wlog(msg: string, extra?: unknown) {
  try { window.nix?.log?.('renderer', msg, extra) } catch {}
  // eslint-disable-next-line no-console
  if (extra !== undefined) console.log('[Nix]', msg, extra); else console.log('[Nix]', msg)
}
...
export interface AgentCall {
  ...
  result?: string
  /** ‹nix:write› only: unified diff of the change, for the diff view in the card. */
  diff?: string
  ...
  for (const [id, conversation] of entries) {
    ...
    const protocolReady =
      ...
      messages.some((message) =>
        ...
        /(?:^|[<{‹⟨«〈＜〈])\s*nix:(?:note|run|write|done)\b/i.test(message.text || ''),
      )
    normalized[id] = {
      ...conversation,
      id,
...
export default function useNix() {
  ...
  const setGptModel = useCallback((slug: string) => {
    ...
    try { localStorage.setItem('nix:model', slug) } catch {}
    // Switch it on this conversation's dedicated ChatGPT tab immediately.
    try { window.nix?.switchModel?.(slug, undefined, convId) } catch {}
  }, [])
  ...
  useEffect(() => {
    try {
      const rawModels = localStorage.getItem('nix:models')
      ...
      const rawIntel = localStorage.getItem('nix:intelByModel')
      const obj = rawIntel ? JSON.parse(rawIntel) : null
      if (obj && typeof obj === 'object') intelByModel.current = obj
      const m = localStorage.getItem('nix:model')
      if (m) setGptModelState(m)
      const intelligence = localStorage.getItem('nix:intelligence')
  ...
  useEffect(() => {
    if (!hasLoaded.current || !window.nix) return
    const t = setTimeout(() => {
      window.nix?.saveData({ conversations, convOrder, projects })
    }, 400)
  ...
  const setPermissionLevel = useCallback((level: PermissionLevel) => {
    ...
    try { localStorage.setItem('nix:permission', level) } catch {}
    wlog(`access level → ${level}`)
  }, [])

  useEffect(() => {
    try {
      const saved = localStorage.getItem('nix:permission') as PermissionLevel | null
  ...
  function dispatchAgentContinuation(continuation: AgentContinuation) {
    ...
    const transport = window.nix?.send
  ...
  function addMessageToConv(convId: string, role: 'user' | 'assistant', data: any) {
    setConversations(prev => {
      ...
      setTimeout(() => {
        ...
        window.nix?.saveData({ conversations: newConvs, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return newConvs
    })
  }
  ...
  useEffect(() => {
    ...
    window.nix?.loadData().then((result: any) => {
    ...
    trackListener(window.nix?.onStatus((data: any) => {
      if (data.type === 'server') {
    ...
    trackListener(window.nix?.onSent((data: any) => {
    ...
    trackListener(window.nix?.onAgentExecProgress?.((data: any) => {
    ...
    trackListener(window.nix?.onModels?.((data: any) => {
      const sourceConvId =
        (typeof data?.convId === 'string' && data.convId) ||
        snap.current.currentConvId ||
        ''
      ...
      // Diagnostic: shows in the Nix app's DevTools console. If `intel` or
      ...
      // eslint-disable-next-line no-console
      console.log('[Nix] onModels ← conv:', sourceConvId || '(none)', '| models:', incoming.length, '| current:', currentModel || '(none)', '| selected-intel:', currentIntelligence || '(none)', '| intel:', intelligences.map(i => i.label).join(',') || '(none)')

      if (currentModel && intelligences.length) {
        intelByModel.current = { ...intelByModel.current, [currentModel]: intelligences }
        try { localStorage.setItem('nix:intelByModel', JSON.stringify(intelByModel.current)) } catch {}
      }
      ...
      setAvailableModels(prev => {
        ...
        try { localStorage.setItem('nix:models', JSON.stringify(withIntel)) } catch {}
        return withIntel
      })
      ...
      if (sourceConvId && (currentModel || currentIntelligence)) {
        ...
        if (sourceConvId === snap.current.currentConvId) {
          if (confirmed.model) {
            ...
            try { localStorage.setItem('nix:model', confirmed.model) } catch {}
          }
          setIntelligenceLevelState(confirmed.intelligence || '')
          if (confirmed.intelligence) {
            try { localStorage.setItem('nix:intelligence', confirmed.intelligence) } catch {}
          }
          setSelectionStatus('confirmed')
        }
      }
    ...
    trackListener(window.nix?.onSelectionError?.((data: any) => {
      const sourceConvId =
        (typeof data?.convId === 'string' && data.convId) ||
        snap.current.currentConvId ||
        ''
    ...
    trackListener(window.nix?.onStreamUpdate((data: any) => {
      const id = typeof data?.convId === 'string' ? data.convId : ''
      if (!id || !snap.current.conversations[id]) {
    ...
    trackListener(window.nix?.onResponse((data: any) => {
    ...
    trackListener(window.nix?.onError((data: any) => {
    ...
    trackListener(window.nix?.onWrongConversation?.((data: any) => {
      ...
      if (!out || !expected || wrongConvRetry.current >= 3) {
        ...
        if (pendingContinuation) {
          setErrorMessage(
            "The browser tab keeps leaving this task. Nix is holding the result and will keep retrying it.",
          )
      ...
      window.nix?.navigate(expected, convId || undefined)
    }))

    trackListener(window.nix?.onDebugResult((data: any) => {
      let body = ''
      if (data.data?.html) {
        body = data.data.html
  ...
  const save = useCallback(() => {
    const s = snap.current
    window.nix?.saveData({ conversations: s.conversations, convOrder: s.convOrder, projects: s.projects })
  }, [])
  ...
  async function addProject() {
    const res = await window.nix?.selectFolder?.()
  ...
  function switchConversation(id: string, skipNavigate?: boolean) {
    ...
    if (!skipNavigate && s?.chatgptUrl && s.chatgptUrl !== actual) {
      ...
      window.nix?.navigate(s.chatgptUrl, id)
  ...
  function send(
    ...
    if (attachments?.length && convId) {
      ...
      window.nix?.sendFiles(convId, attachments)
    }
  ...
  useEffect(() => {
    ...
    for (const [convId, files] of queuedFiles.current) {
      window.nix?.sendFiles(convId, files)
    }
  ...
  function handleAgentResponse(convId: string, rawText: string): boolean {
    ...
    // ‹nix:done› is terminal and takes PRECEDENCE. If the model wrapped a final
    ...
    if (parsedToolActions.length === 0) {
      const protocolLike = /(?:^|[<{‹⟨«〈＜〈])\s*\/?\s*nix\s*:/im.test(rawText)
      const partialDone =
        /(?:^|[<{‹⟨«〈＜〈])\s*nix:done\b/im.test(rawText)
      ...
      if (protocolLike && retryCount < 2) {
        ...
        const wire =
          '{nix:result kind="protocol" status="error"}\n' +
          (partialDone
            ? 'Your final answer was truncated. Re-emit the complete answer now in one concise {nix:done} block. Do not run tools or add action tags.\n'
            : 'Your previous action response was incomplete. Re-emit that same action turn now with one complete {nix:note} and no more than 6 complete action tags. Do not repeat completed work.\n') +
          '{/nix:result}'
    ...
    if (loopCount > MAX_AGENT_LOOP) {
      ...
      const wire =
        '{nix:result kind="protocol" status="error"}\n' +
        `${limitMessage} Reply with one complete {nix:done} block and no action tags.\n` +
        '{/nix:result}'
  ...
  // combined ‹nix:result› blocks back into the thread as an invisible turn.
  async function executeTurn(convId: string, toolActions: ToolAction[], decide: (a: ToolAction) => Disposition) {
    ...
    const executeAction = async (origIdx: number) => {
      ...
      try {
        const response = await window.nix?.agentExec?.({
          cwd,
    ...
    const wire =
      ...
      '{nix:result kind="guidance" status="ok"}\n' +
  ...
  function stopSending() {
    ...
    try { window.nix?.stopGenerating?.(stopConv) } catch {}
    activeSendConv.current = null
    scheduleQueuedUserSend()
  }
  ...
  function setArchived(ids: string[], archived: boolean) {
    ...
    setConversations(prev => {
      ...
      setTimeout(() => {
        const s = snap.current
        window.nix?.saveData({ conversations: updated, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return updated
    })
  ...
  function deleteConv(ids: string[]) {
    ...
    setConversations(prev => {
      ...
      setTimeout(() => {
        const s = snap.current
        window.nix?.saveData({ conversations: rest, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return rest
    })
  ...
  function deleteAllConversations() {
    ...
    setTimeout(() => {
      window.nix?.saveData({ conversations: {}, convOrder: [], projects: snap.current.projects })
    }, 0)
  }
