...
import type { AgentCall, Conversation, Message } from '../../hooks/useNix'
...
describe('MessageLog work transcript', () => {
  test('groups commands beneath compact phase disclosures', async () => {
    ...
    expect(document.querySelector('.nix-code')).toHaveTextContent('{}')
  })
  ...
  test('hides historical retry duplicates and never exposes partial protocol text', () => {
    ...
    expect(screen.queryByText('{nix:run}cat package.json')).not.toBeInTheDocument()
    ...
    render(
      <MessageLog
        conversation={conversation([
          ...
          repeated,
          repeated,
          { role: 'assistant', text: "{nix:note}I'll map the project structure first{/nix" },
          repeated,
  ...
  test('shimmers only the latest activity phase while work is in progress', () => {
    ...
    expect(screen.getByText('Reading the repository structure')).not.toHaveClass('nix-shimmer')
    expect(screen.getByText('Understanding potential next steps')).toHaveClass('nix-shimmer')
  })
    ...
    rerender(
      <MessageLog
        ...
        conversation={conversation([
          ...
          {
            role: 'assistant',
            text: '{nix:note}Understanding potential',
            streaming: true,
          },
        ])}
      />,
  ...
  test('uses one visual row contract for thinking, phases, and commands', async () => {
    ...
    expect(thinking).toHaveClass('nix-activity-row', 'nix-thinking-row')
    expect(thinking?.querySelector('.nix-thinking-label')).toHaveClass('nix-thinking-pulse')
    expect(thinking?.querySelector('.nix-activity-tertiary')).toHaveClass('h-6', 'w-[4.25rem]')
    expect(thinking?.querySelector('.nix-shimmer')).toHaveTextContent('Thinking')
    ...
    expect(phase).toHaveClass('nix-activity-row')
    expect(phase).toHaveClass('nix-phase-trigger', 'gap-1')
    expect(phase).not.toHaveClass('nix-activity-interactive')
    const phaseTitle = phase?.querySelector('.nix-phase-title')
    ...
    expect(tool).toHaveClass('nix-activity-row')
  })
  ...
  test('renders denied command status with the destructive color', async () => {
    ...
    deniedMarks.forEach(mark => {
      expect(mark).toHaveClass('nix-tool-status-error')
      expect(mark).not.toHaveClass('nix-activity-tertiary')
    })
  })
  ...
  test('syntax highlights source printed by a shell file-read command', async () => {
    ...
    expect(output).toHaveClass('nix-code')
    ...
    expect(output?.closest('[data-activity-rule]')).toHaveClass('nix-tool-result-surface')
  })
