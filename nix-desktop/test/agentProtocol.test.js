...
test('parser preserves narration and calls in emission order', () => {
  const result = protocol.parseAgentActions(
    '{nix:note}I will inspect the entry points next.{/nix:note}\n' +
      '{nix:run}cat package.json{/nix:run}\n' +
      '{nix:run}sed -n "1,160p" src/index.ts{/nix:run}',
  );
  assert.deepEqual(
...
test('parser does not promote a truncated action to an executable call', () => {
  const result = protocol.parseAgentActions('{nix:run}cat package.json');
  assert.equal(result.hasDone, false);
  assert.deepEqual(result.actions, []);
  assert.equal(protocol.stripAgentTags('{nix:run}cat package.json'), '');
});

test('parser accepts the canonical completed-answer wrapper', () => {
  const result = protocol.parseAgentActions(
    '{nix:done}The project has two runtime components.{/nix:done}',
  );
...
test('parser preserves explicit human-review requests on actions', () => {
  const result = protocol.parseAgentActions(
    '{nix:run approval="required"}pnpm install{/nix:run}\n' +
      '{nix:write path="README.md" approval="required"}content{/nix:write}',
  );
  assert.deepEqual(result.actions, [
...
test('parser tolerates spaced tags and decodes escaped attributes', () => {
  const result = protocol.parseAgentActions(
    '{ nix:search query="&quot;message&quot;" path="src" /}',
  );
  ...
  assert.equal(protocol.stripAgentTags('{ nix:search query="hello" /}'), '');
});
