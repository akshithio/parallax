...
function FilesSurface({ cwd }: { cwd: string | null }) {
  ...
  const openFile = useCallback(
    async (path: string) => {
      ...
      if (BINARY_EXT.test(path)) {
        setFile({ status: 'error', message: "This looks like a binary file — Nix won't render it as text." })
        return
      }
      setFile({ status: 'loading' })
      try {
        const res = await window.nix?.agentExec?.({ cwd, actions: [{ type: 'read', path }] })
  ...
  const loadDir = useCallback(
    async (path: string) => {
      ...
      try {
        const res = await window.nix?.agentExec?.({ cwd, actions: [{ type: 'list', path: path || '.' }] })
        const r = res?.results?.[0]
        setDirs((prev) => ({
          ...prev,
          [path]:
