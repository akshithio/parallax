...
// `nix-log` IPC; the content script relays over the WebSocket (`type:"log"`).
// One stream, whole flow, no DevTools spelunking across three consoles.
//
// The same stream is TEE'D TO A FILE (~/Library/Logs/Nix/nix.log). A terminal
...
// misbehaves.
const LOG_DIR = path.join(os.homedir(), 'Library', 'Logs', 'Nix');
const LOG_FILE = path.join(LOG_DIR, 'nix.log');
let logStream = null;
function openLogFile() {
  try {
    ...
    logStream.write(`# Nix session started ${new Date().toISOString()}\n`);
  } catch (_) {
    logStream = null;
  }
}
function wlog(scope, msg, extra) {
  ...
  const line = `[nix ${ts}] ${scope.padEnd(8)} ${msg}`;
...
function createWindow() {
  ...
  mainWindow.webContents.on('will-attach-webview', (_event, webPreferences, params) => {
    ...
    if (typeof params.partition !== 'string' || !params.partition.startsWith('persist:nix-preview')) {
      params.partition = 'persist:nix-preview';
    }
  });
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    // do nothing — pageReady is set by renderer's nix-ready signal
  });
}
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'models': {
            ...
            console.log(`[nix] models: conv=${msg.convId || '-'} | ${mModels.length} models | current=${mCurrent || '-'} | selected-intel=${mCurrentIntel || '-'} | intel=${mIntel.map((i) => i && i.label).filter(Boolean).join(',') || '-'}`);
...
function watchExtensionForReload() {
  ...
  const extDir = path.join(__dirname, '..', 'nix-extension');
...
ipcMain.on('nix-log', (_event, { scope, msg, extra }) => wlog(scope || 'renderer', msg, extra));
...
ipcMain.on('send-message', (_event, { text, model, intelligence, wireText, silent, expectUrl, convId, msgId: requestedMsgId }) => {
  ...
  if (!sendToExtension({ type: 'send_message', text: outgoing, msgId, model, intelligence, expectUrl, convId })) {
    ...
    sendToRenderer('error', {
      convId,
      msgId,
      message: 'No Chrome extension connected. Open ChatGPT and check the Nix popup.',
    });
    return;
  }
});
...
ipcMain.on('nix-ready', () => {
  ...
  wlog('ipc', `nix-ready → status snapshot: ws=${connected ? 'connected' : 'disconnected'} chats=${lastReadyUrls.size}`);
  sendToRenderer('status', {
    type: 'server',
...
ipcMain.handle('preview-clear-cookies', async () => {
  await session.fromPartition('persist:nix-preview').clearStorageData({ storages: ['cookies'] });
  return { ok: true };
});

ipcMain.handle('preview-clear-cache', async () => {
  await session.fromPartition('persist:nix-preview').clearCache();
  return { ok: true };
});
...
ipcMain.handle('preview-save-recording', async (_event, payload) => {
  ...
  const result = await dialog.showSaveDialog({
    ...
    defaultPath: path.join(app.getPath('downloads'), `nix-browser-${stamp}.${extension}`),
