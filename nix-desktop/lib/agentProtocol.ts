// Parser for the Nix agent protocol emitted by the model.
//
// Delimiters: the canonical form is BRACES { } — {nix:run}…{/nix:run}. Braces
// aren't special in Markdown or HTML, so a bare tag survives a DOM scrape intact
// (ChatGPT's renderer parses <nix:run> into an element and the delimiters vanish),
...
// HTML, so a bare {nix:run} survives a DOM scrape intact (angle brackets don't —
// the browser parses <nix:run> into an element and the delimiters vanish).
//
...
// tag to its opener. The close allows stray whitespace (‹ / nix:run ›) because
// the model sometimes spaces it out.
const TAG_RE = new RegExp(
  `[${OPEN}]\\s*nix:(note|read|list|search|run|write|done)\\b([^${CLOSE}]*?)` +
    `(?:\\/[${CLOSE}]|[${CLOSE}]([\\s\\S]*?)[${OPEN}]\\s*\\/\\s*nix:\\1\\s*[${CLOSE}])`,
  'g',
)
...
export interface ToolResult {
  status: 'ok' | 'error'
  content: string
  /** For ‹nix:write›: a unified diff of the change, shown in the UI only (never
...
/** Serialize executed tool results into the {nix:result} blocks fed back to the model. */
...
function truncateResult(content: string, budget: number): string {
  ...
  const marker =
    `\n… [truncated by Nix — ${budget} of ${content.length} characters shown. ` +
...
export function formatAgentResults(actions: AgentAction[], results: ToolResult[]): string {
  ...
  return actions
    .map((a, i) => {
      ...
      return `{nix:result ${attrs} status="${r.status}"}\n${body}\n{/nix:result}`
    })
    .join('\n\n')
}

/** Remove nix protocol tags (any delimiter, complete/self-closing/partial/mangled) from display text. */
export function stripAgentTags(text: string): string {
  ...
  const O = OPEN
  const C = CLOSE
  // A paired ‹nix:TAG›…‹/nix:TAG›. `tag` may be a literal ("done") or an
  // alternation whose close backreferences \1 ("(note|read|…)").
  const paired = (tag: string) =>
    new RegExp(
      `[${O}]\\s*nix:${tag}\\b[^${C}]*[${C}]([\\s\\S]*?)[${O}]\\s*\\/\\s*nix:${tag.startsWith('(') ? '\\1' : tag}\\s*[${C}]`,
      'g',
    )
  return (
    text
      // ‹nix:done›…final answer…‹/nix:done› — UNWRAP: keep the answer, drop the tag.
      .replace(paired('done'), '$1')
      // ‹nix:result›… blocks are the harness's; if echoed into the answer, drop them.
      .replace(paired('result'), '')
      .replace(paired('(note|read|list|search|run|write)'), '')
      .replace(new RegExp(`[${O}]\\s*nix:(?:note|read|list|search|run|write|done)\\b[^${C}]*\\/[${C}]`, 'g'), '')
      // STREAMING: ‹nix:done› opened but hasn't closed yet — unwrap the partial
      // answer so it renders token-by-token instead of appearing all at once.
      .replace(new RegExp(`[${O}]\\s*nix:done\\b[^${C}]*[${C}]([\\s\\S]*)$`), '$1')
      ...
      .replace(
        new RegExp(
          `[${O}]\\s*nix:(?:note|read|list|search|run|write)\\b[^${C}]*[${C}][\\s\\S]*$`,
        ),
        '',
      )
      .replace(new RegExp(`[${O}]\\s*\\/?\\s*nix:[a-z]+[^${C}]*[${C}]`, 'g'), '')
      .replace(new RegExp(`[${O}]\\s*nix:[\\s\\S]*$`, 'g'), '')
      // A tag opener cut mid-token at the very end — "{w", "{we", "{/nix". A `w`
