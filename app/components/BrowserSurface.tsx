...
export default function BrowserSurface({ cwd, scopeKey }: Props) {
  ...
  const persistKey = `parallax:preview:${scopeKey || 'default'}`
  ...
  const refreshServers = useCallback(async () => {
    try {
      const result = await window.parallax?.previewListServers?.()
  ...
  const completeAnnotation = useCallback(async (payload: any) => {
    ...
    try {
      ...
      view.send('parallax-preview-annotation-captured')
  ...
  useEffect(() => {
    ...
    const onIpc = (event: any) => {
      if (event?.channel !== 'parallax-preview-annotation') return
  ...
  useEffect(() => {
    const unsubscribe = window.parallax?.onPreviewRecordingFrame?.((frame: any) => {
      const view = viewRef.current
  ...
  const toggleAnnotation = useCallback(() => {
    ...
    if (!view || !current) return
    if (annotationActive) {
      view.send('parallax-preview-annotation-cancel')
      setAnnotationActive(false)
    } else {
      view.send('parallax-preview-annotation-start', readTheme())
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      if (recordingWebContentsId.current != null) {
        await window.parallax?.previewStopRecording?.(recordingWebContentsId.current)
      }
    } catch {}
    ...
    if (blob.size > 0) {
      ...
      const result = await window.parallax?.previewSaveRecording?.(data, blob.type)
  ...
  const toggleRecording = useCallback(async () => {
    ...
    try {
      ...
      await window.parallax?.previewStartRecording?.(webContentsId)
  ...
  useEffect(
    () => () => {
      ...
      if (recordingWebContentsId.current != null) {
        void window.parallax?.previewStopRecording?.(recordingWebContentsId.current)
      }
      recordingWebContentsId.current = null
    },
    [scopeKey],
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="relative flex items-center gap-1 border-b border-border/60 px-1.5 py-1.5">
        ...
        <button type="button" disabled={!current} onClick={() => window.parallax?.previewOpenExternal?.(current)} className={buttonClass()} title="Open in browser">
          <ExternalIcon />
        </button>
        ...
        <div ref={moreRef} className="relative">
          ...
          {moreOpen && (
            <div className="absolute right-0 top-full z-50 mt-1.5 w-56 rounded-xl border border-border bg-popover p-1.5 text-[12px] shadow-xl">
              ...
              <div className="my-1 h-px bg-border" />
              <MenuButton onClick={async () => { setMoreOpen(false); await window.parallax?.previewClearCookies?.(); viewRef.current?.reload() }}>Clear cookies</MenuButton>
              <MenuButton onClick={async () => { setMoreOpen(false); await window.parallax?.previewClearCache?.(); viewRef.current?.reloadIgnoringCache() }}>Clear cache</MenuButton>
            </div>
          )}
        </div>
      </div>

      {loading && <div className="parallax-progress h-0.5 w-full" />}
        ...
        <div ref={viewportRef} className={cn('relative min-h-0 flex-1 overflow-auto', viewport && 'bg-secondary/45 p-5')}>
          <div className={cn('relative mx-auto bg-white shadow-sm', !viewport && 'h-full')} style={currentViewportStyle}>
            <webview
              ...
              ref={viewRef as any}
              src={current}
              partition="persist:parallax-preview"
              allowpopups
            ...
            <div className="absolute inset-0 grid place-items-center bg-background p-6 text-center">
              <div className="max-w-[250px]">
                ...
                <p className="mt-1 text-[11.5px] leading-relaxed text-muted-foreground/80">
                  Quit and reopen Parallax so the embedded browser preload can initialize.
                </p>
              </div>
            </div>
          )}
