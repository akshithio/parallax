...
Parallax Desktop is an Electron-based coding-agent interface that connects a local project workspace to ChatGPT through the Parallax browser extension.
...
- pnpm 10
- Google Chrome
- The Parallax Chrome extension installed and enabled
...
`latest-mac.yml` metadata that installed copies of Parallax use to discover updates.

The updater:

- checks shortly after an installed build starts and every four hours afterward;
- downloads a newer release in the background;
- installs the downloaded release when Parallax quits; and
...
On the first message of a conversation, Parallax prepends an agent-system prompt to the message sent to ChatGPT.
