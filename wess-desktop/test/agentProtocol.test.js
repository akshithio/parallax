...
test('parser preserves narration and calls in emission order', () => {
  const result = protocol.parseAgentActions(
    '{wess:note}I will inspect the entry points next.{/wess:note}\n' +
      '{wess:run}cat package.json{/wess:run}\n' +
      '{wess:run}sed -n "1,160p" src/index.ts{/wess:run}',
  );
  assert.deepEqual(
...
test('parser does not promote a truncated action to an executable call', () => {
  const result = protocol.parseAgentActions('{wess:run}cat package.json');
  assert.equal(result.hasDone, false);
  assert.deepEqual(result.actions, []);
  assert.equal(protocol.stripAgentTags('{wess:run}cat package.json'), '');
});

test('parser accepts the canonical completed-answer wrapper', () => {
  const result = protocol.parseAgentActions(
    '{wess:done}The project has two runtime components.{/wess:done}',
  );
...
test('parser preserves explicit human-review requests on actions', () => {
  const result = protocol.parseAgentActions(
    '{wess:run approval="required"}pnpm install{/wess:run}\n' +
      '{wess:write path="README.md" approval="required"}content{/wess:write}',
  );
  assert.deepEqual(result.actions, [
...
test('parser tolerates spaced tags and decodes escaped attributes', () => {
  const result = protocol.parseAgentActions(
    '{ wess:search query="&quot;message&quot;" path="src" /}',
  );
  ...
  assert.equal(protocol.stripAgentTags('{ wess:search query="hello" /}'), '');
});
