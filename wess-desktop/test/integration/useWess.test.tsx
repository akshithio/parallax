...
import useWess, { type Conversation } from '../../hooks/useWess'
import { HARNESS_PROTOCOL_VERSION } from '../../lib/systemPrompt'
import { createWessBridge } from '../helpers/wessBridge'
...
describe('useWess transport integration', () => {
  beforeEach(() => {
    localStorage.clear()
  })

  test('shows a message immediately and reconciles acknowledgement without duplication', async () => {
    const bridge = createWessBridge(data({ 'thread-a': thread('thread-a') }))
    window.wess = bridge.api
    const { result } = renderHook(() => useWess())
  ...
  test('queues user messages during an active turn and drains them in order', async () => {
    const bridge = createWessBridge(data({ 'thread-a': thread('thread-a', URL_A) }))
    window.wess = bridge.api
    const { result } = renderHook(() => useWess())
  ...
  test('edits a user message by replacing its branch in the owned browser conversation', async () => {
    ...
    const bridge = createWessBridge(data({ 'thread-a': existing }))
    window.wess = bridge.api
    const { result } = renderHook(() => useWess())
  ...
  test('sends a greeting normally and delays the workspace protocol until it is needed', async () => {
    ...
    expect(workspaceRequest[3]).toContain('{wess:task}\nInspect this repository.\n{/wess:task}')
  ...
  test('Manual synchronously presents and denies one shell command at a time', async () => {
    ...
    act(() => {
      ...
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Reading the repository structure{/wess:note}\n' +
          '{wess:run}ls -la{/wess:run}\n' +
          '{wess:run}pwd{/wess:run}',
      })
    })
  ...
  test('Auto-review raises explicitly flagged actions for human approval', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Updating the project{/wess:note}\n' +
          '{wess:write path="README.md" approval="required"}Viewer documentation{/wess:write}',
      })
    })
  ...
  test('Auto-review executes an unflagged routine change without pausing', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        text:
          '{wess:note}Updating the project{/wess:note}\n' +
          '{wess:write path="README.md"}Viewer documentation{/wess:write}',
      })
    })
  ...
  test('publishes a running command body before the process returns', async () => {
    ...
    act(() => {
      ...
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Checking the workspace{/wess:note}\n' +
          '{wess:run}printf ready{/wess:run}',
      })
    })
  ...
  test('shows a streamed write batch before execution and always returns its results', async () => {
    ...
    const { result } = renderHook(() => useWess())
    const actionTurn =
      '{wess:note}Creating the application foundation{/wess:note}\n' +
      '{wess:write path="package.json"}{"name":"viewer"}{/wess:write}\n' +
      '{wess:write path="tsconfig.json"}{"compilerOptions":{}}{/wess:write}\n' +
      '{wess:write path="next-env.d.ts"}/// reference types="next"{/wess:write}\n' +
      '{wess:write path="next.config.mjs"}export default {}{/wess:write}'
  ...
  test('returns an executor failure to the browser instead of abandoning the turn', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Reading project metadata{/wess:note}\n' +
          '{wess:read path="package.json" /}',
      })
    })
  ...
  test('turns the action-round cap into a forced final-answer continuation', async () => {
    ...
    for (let index = 0; index < 31; index += 1) {
      act(() => {
        bridge.emit('response', {
          ...
          url: URL_A,
          text:
            `{wess:note}Reading action round ${index + 1}{/wess:note}\n` +
            `{wess:read path="file-${index + 1}.txt" /}`,
        })
      })
      await waitFor(() =>
    ...
    expect(finalContinuation?.[3]).toContain('Reply with one complete {wess:done} block')
    expect(
  ...
  test('removes transport listeners on unmount before a replacement hook subscribes', async () => {
    ...
    const first = renderHook(() => useWess())
    ...
    const replacement = renderHook(() => useWess())
    ...
    act(() => {
      bridge.emit('response', {
        convId: 'thread-a',
        url: URL_A,
        text: '{wess:done}Finished once.{/wess:done}',
      })
    })
    ...
    expect(replacement.result.current.conversations['thread-a'].messages[0].text).toBe(
      '{wess:done}Finished once.{/wess:done}',
    )
  })
  ...
  test('preserves each action round when executor results arrive immediately', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          "{wess:note}I'll inspect the metadata first.{/wess:note}\n" +
          '{wess:read path="package.json" /}',
      })
    })
    await waitFor(() =>
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          "{wess:note}I'll inspect the components next.{/wess:note}\n" +
          '{wess:list path="components" /}',
      })
    })
  ...
  test('runs independent reads together and publishes each result as it finishes', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Reading both files{/wess:note}\n' +
          '{wess:read path="first.txt" /}\n' +
          '{wess:read path="second.txt" /}',
      })
    })
  ...
  test('publishes running command output before the final result', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Reading top-level files{/wess:note}\n' +
          '{wess:run}find . -maxdepth 1 -type f{/wess:run}',
      })
    })
  ...
  test('recovers a truncated final answer without running more tools', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        convId: 'thread-a',
        url: URL_A,
        text: '{wess:done}This answer was cut off',
      })
    })
  ...
  test('keeps follow-up turn boundaries isolated between background tasks', async () => {
    ...
    act(() => {
      bridge.emit('response', {
        ...
        url: URL_A,
        text:
          '{wess:note}Reading first task{/wess:note}\n' +
          '{wess:read path="first.txt" /}',
      })
      bridge.emit('response', {
        convId: 'thread-b',
        url: URL_B,
        text:
          '{wess:note}Reading second task{/wess:note}\n' +
          '{wess:read path="second.txt" /}',
      })
    })
    await waitFor(() =>
    ...
    act(() => {
      bridge.emit('stream_update', {
        convId: 'thread-a',
        text: '{wess:done}First task finished.',
      })
      bridge.emit('stream_update', {
        convId: 'thread-b',
        text: '{wess:done}Second task finished.',
      })
    })
