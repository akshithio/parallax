...
import type { AgentCall, Conversation, Message } from '../../hooks/useWess'
...
describe('MessageLog work transcript', () => {
  test('groups commands beneath compact phase disclosures', async () => {
    ...
    expect(document.querySelector('.wess-code')).toHaveTextContent('{}')
  })
  ...
  test('hides historical retry duplicates and never exposes partial protocol text', () => {
    render(
      <MessageLog
        conversation={conversation([
          ...
          {
            role: 'assistant',
            text: '{wess:run}cat package.json',
            streaming: false,
          },
        ])}
      />,
    ...
    expect(screen.queryByText('{wess:run}cat package.json')).not.toBeInTheDocument()
    ...
    render(
      <MessageLog
        conversation={conversation([
          ...
          repeated,
          repeated,
          { role: 'assistant', text: "{wess:note}I'll map the project structure first{/wess" },
          repeated,
  ...
  test('shimmers only the latest activity phase while work is in progress', () => {
    ...
    expect(screen.getByText('Reading the repository structure')).not.toHaveClass('wess-shimmer')
    expect(screen.getByText('Understanding potential next steps')).toHaveClass('wess-shimmer')
  })
    ...
    rerender(
      <MessageLog
        ...
        conversation={conversation([
          ...
          {
            role: 'assistant',
            text: '{wess:note}Understanding potential',
            streaming: true,
          },
        ])}
      />,
  ...
  test('uses one visual row contract for thinking, phases, and commands', async () => {
    ...
    expect(thinking).toHaveClass('wess-activity-row', 'wess-thinking-row')
    expect(thinking?.querySelector('.wess-thinking-label')).toHaveClass('wess-thinking-pulse')
    expect(thinking?.querySelector('.wess-activity-tertiary')).toHaveClass('h-6', 'w-[4.25rem]')
    expect(thinking?.querySelector('.wess-shimmer')).toHaveTextContent('Thinking')
    ...
    expect(phase).toHaveClass('wess-activity-row')
    expect(phase).toHaveClass('wess-phase-trigger', 'gap-1')
    expect(phase).not.toHaveClass('wess-activity-interactive')
    const phaseTitle = phase?.querySelector('.wess-phase-title')
    ...
    expect(tool).toHaveClass('wess-activity-row')
  })
  ...
  test('renders denied command status with the destructive color', async () => {
    ...
    deniedMarks.forEach(mark => {
      expect(mark).toHaveClass('wess-tool-status-error')
      expect(mark).not.toHaveClass('wess-activity-tertiary')
    })
  })
  ...
  test('syntax highlights source printed by a shell file-read command', async () => {
    ...
    expect(output).toHaveClass('wess-code')
    ...
    expect(output?.closest('[data-activity-rule]')).toHaveClass('wess-tool-result-surface')
  })
