...
async function emit(page: Page, event: string, payload: unknown) {
  await page.evaluate(
    ({ eventName, eventPayload }) => {
      window.__wessHarness.emit(eventName, eventPayload)
    },
...
test('runs a naturally ordered two-round repository workflow', async ({ page }) => {
  await page.addInitScript(
    ({ convId, chatUrl }) => {
      ...
      window.__wessHarness = harness
      localStorage.setItem('wess:permission', 'full-access')
  ...
  await expect(page.locator('.wess-thinking-enter')).toHaveCSS('transform', 'none')
  const thinkingAnimation = await page.locator('.wess-thinking-pulse').evaluate((element) => {
    const style = getComputedStyle(element)
    return {
  ...
  expect(thinkingAnimation).toEqual({
    name: 'wess-thinking-pulse',
    duration: '1.5s',
    iterations: 'infinite',
  })
  const shimmerAnimation = await page.locator('[aria-label="Thinking"] .wess-shimmer').evaluate((element) => {
    const style = getComputedStyle(element)
    return {
      name: style.animationName,
      duration: style.animationDuration,
  ...
  const thinkingLabelMetrics = await page.locator('.wess-thinking-pulse').evaluate((element) => {
  ...
  const settledThinkingLabel = await page.locator('.wess-thinking-pulse').boundingBox()
  ...
  const executionsBeforeActionStream = await page.evaluate(
    () => window.__wessHarness.executions.length,
  )
  ...
  expect(await page.evaluate(() => window.__wessHarness.executions.length)).toBe(
  ...
  const phaseTitleMetrics = await structurePhase.locator('.wess-phase-title').evaluate((element) => {
    const box = element.getBoundingClientRect()
    const style = getComputedStyle(element)
  ...
  const transcriptScroller = page.locator('[data-wess-transcript]')
  ...
  await emit(page, 'response', {
    ...
    url: CHAT_URL,
    text:
      '{wess:note}Tracing message delivery{/wess:note}\n' +
  ...
  await emit(page, 'response', {
    ...
    url: CHAT_URL,
    text:
      '{wess:done}The desktop routes each task through its own browser conversation and renders action rounds as an ordered transcript.{/wess:done}',
  })
  ...
  const answerMetrics = await transcriptMetrics(
    page.locator('.wess-transcript-copy').filter({ hasText: finalAnswer }),
  )
  ...
  const answerCopy = page.locator('.wess-transcript-copy').filter({ hasText: finalAnswer })
  ...
  const workflowExecutions = await page.evaluate(() =>
    window.__wessHarness.executions.slice(-4).map((batch) =>
  ...
  const approvalBox = await page.locator('.wess-approval-surface').boundingBox()
  ...
  const approvalSurface = page.locator('.wess-approval-surface')
