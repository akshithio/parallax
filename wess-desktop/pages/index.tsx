import useWess from '../hooks/useWess'
...
export default function Home() {
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if ((e.metaKey || e.ctrlKey) && !e.shiftKey && e.key === 'a') {
        ...
        const transcript = document.querySelector('[data-wess-transcript]')
  ...
  useEffect(() => {
    window.__wess_project_cwd__ = currentFolder || ''
  }, [currentFolder])
  ...
  const checkGit = useCallback(async () => {
    ...
    try {
      const res = await window.wess?.agentExec?.({ cwd: currentFolder, actions: [{ type: 'list', path: '.' }] })
  ...
  const handleOpen = useCallback(
    (editorId: string) => {
      ...
      try { localStorage.setItem('wess:preferredEditor', editorId) } catch {}
      window.wess?.openInEditor?.(editorId, currentFolder)
    },
    [currentFolder],
  )
  ...
  const paletteCommands: Command[] = useMemo(() => {
    const setTheme = (mode: 'light' | 'dark') => {
      try { localStorage.setItem('wess:theme', mode) } catch {}
  ...
  return (
    <div className="relative isolate flex h-screen overflow-hidden bg-background text-foreground max-md:flex-col">
      ...
      >
        ...
        <div className="wess-main-panel flex min-h-0 flex-1 flex-col" data-main-panel>
        {/* Main chat column */}
        {w.currentConv ? (
          <>
            <MessageLog
                ...
                <div className="mx-auto w-full max-w-3xl space-y-1.5">
                  {w.extensionDisconnected && (
                    <ComposerBanner
                      ...
                      detail={
                        w.queuedNotice
                          ? 'Your message is queued — it’ll send automatically the moment the Wess extension reconnects.'
                          : 'Open ChatGPT and check the Wess extension. You can keep typing here — messages send as soon as it reconnects.'
                      }
                    />
                  )}
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="max-w-sm text-sm text-muted-foreground/78">
                Pick a folder to work in. Wess reads and reasons over it, and your threads live inside the project.
              </p>
            </div>
            <button
              type="button"
      ...
      <SettingsModal
        ...
        onCheckForUpdates={() => { void window.wess?.checkForUpdates?.() }}
        onInstallUpdate={() => window.wess?.installUpdate?.()}
      />
    </div>
  )
}
