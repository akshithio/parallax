...
declare global {
  interface Window { wess: any; __wess_project_cwd__?: string }
}
...
function wlog(msg: string, extra?: unknown) {
  try { window.wess?.log?.('renderer', msg, extra) } catch {}
  // eslint-disable-next-line no-console
  if (extra !== undefined) console.log('[Wess]', msg, extra); else console.log('[Wess]', msg)
}
...
export interface AgentCall {
  ...
  result?: string
  /** ‹wess:write› only: unified diff of the change, for the diff view in the card. */
  diff?: string
  ...
  for (const [id, conversation] of entries) {
    ...
    const protocolReady =
      ...
      messages.some((message) =>
        ...
        /(?:^|[<{‹⟨«〈＜〈])\s*wess:(?:note|run|write|done)\b/i.test(message.text || ''),
      )
    normalized[id] = {
      ...conversation,
      id,
...
export default function useWess() {
  ...
  const setGptModel = useCallback((slug: string) => {
    ...
    try { localStorage.setItem('wess:model', slug) } catch {}
    // Switch it on this conversation's dedicated ChatGPT tab immediately.
    try { window.wess?.switchModel?.(slug, undefined, convId) } catch {}
  }, [])
  ...
  useEffect(() => {
    try {
      const rawModels = localStorage.getItem('wess:models')
      ...
      const rawIntel = localStorage.getItem('wess:intelByModel')
      const obj = rawIntel ? JSON.parse(rawIntel) : null
      if (obj && typeof obj === 'object') intelByModel.current = obj
      const m = localStorage.getItem('wess:model')
      if (m) setGptModelState(m)
      const intelligence = localStorage.getItem('wess:intelligence')
  ...
  useEffect(() => {
    if (!hasLoaded.current || !window.wess) return
    const t = setTimeout(() => {
      window.wess?.saveData({ conversations, convOrder, projects })
    }, 400)
  ...
  const setPermissionLevel = useCallback((level: PermissionLevel) => {
    ...
    try { localStorage.setItem('wess:permission', level) } catch {}
    wlog(`access level → ${level}`)
  }, [])

  useEffect(() => {
    try {
      const saved = localStorage.getItem('wess:permission') as PermissionLevel | null
  ...
  function dispatchAgentContinuation(continuation: AgentContinuation) {
    ...
    const transport = window.wess?.send
  ...
  function addMessageToConv(convId: string, role: 'user' | 'assistant', data: any) {
    setConversations(prev => {
      ...
      setTimeout(() => {
        ...
        window.wess?.saveData({ conversations: newConvs, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return newConvs
    })
  }
  ...
  useEffect(() => {
    ...
    window.wess?.loadData().then((result: any) => {
    ...
    trackListener(window.wess?.onStatus((data: any) => {
      if (data.type === 'server') {
    ...
    trackListener(window.wess?.onSent((data: any) => {
    ...
    trackListener(window.wess?.onAgentExecProgress?.((data: any) => {
    ...
    trackListener(window.wess?.onModels?.((data: any) => {
      const sourceConvId =
        (typeof data?.convId === 'string' && data.convId) ||
        snap.current.currentConvId ||
        ''
      ...
      // Diagnostic: shows in the Wess app's DevTools console. If `intel` or
      ...
      // eslint-disable-next-line no-console
      console.log('[Wess] onModels ← conv:', sourceConvId || '(none)', '| models:', incoming.length, '| current:', currentModel || '(none)', '| selected-intel:', currentIntelligence || '(none)', '| intel:', intelligences.map(i => i.label).join(',') || '(none)')

      if (currentModel && intelligences.length) {
        intelByModel.current = { ...intelByModel.current, [currentModel]: intelligences }
        try { localStorage.setItem('wess:intelByModel', JSON.stringify(intelByModel.current)) } catch {}
      }
      ...
      setAvailableModels(prev => {
        ...
        try { localStorage.setItem('wess:models', JSON.stringify(withIntel)) } catch {}
        return withIntel
      })
      ...
      if (sourceConvId && (currentModel || currentIntelligence)) {
        ...
        if (sourceConvId === snap.current.currentConvId) {
          if (confirmed.model) {
            ...
            try { localStorage.setItem('wess:model', confirmed.model) } catch {}
          }
          setIntelligenceLevelState(confirmed.intelligence || '')
          if (confirmed.intelligence) {
            try { localStorage.setItem('wess:intelligence', confirmed.intelligence) } catch {}
          }
          setSelectionStatus('confirmed')
        }
      }
    ...
    trackListener(window.wess?.onSelectionError?.((data: any) => {
      const sourceConvId =
        (typeof data?.convId === 'string' && data.convId) ||
        snap.current.currentConvId ||
        ''
    ...
    trackListener(window.wess?.onStreamUpdate((data: any) => {
      const id = typeof data?.convId === 'string' ? data.convId : ''
      if (!id || !snap.current.conversations[id]) {
    ...
    trackListener(window.wess?.onResponse((data: any) => {
    ...
    trackListener(window.wess?.onError((data: any) => {
    ...
    trackListener(window.wess?.onWrongConversation?.((data: any) => {
      ...
      if (!out || !expected || wrongConvRetry.current >= 3) {
        ...
        if (pendingContinuation) {
          setErrorMessage(
            "The browser tab keeps leaving this task. Wess is holding the result and will keep retrying it.",
          )
      ...
      window.wess?.navigate(expected, convId || undefined)
    }))

    trackListener(window.wess?.onDebugResult((data: any) => {
      let body = ''
      if (data.data?.html) {
        body = data.data.html
  ...
  const save = useCallback(() => {
    const s = snap.current
    window.wess?.saveData({ conversations: s.conversations, convOrder: s.convOrder, projects: s.projects })
  }, [])
  ...
  async function addProject() {
    const res = await window.wess?.selectFolder?.()
  ...
  function switchConversation(id: string, skipNavigate?: boolean) {
    ...
    if (!skipNavigate && s?.chatgptUrl && s.chatgptUrl !== actual) {
      ...
      window.wess?.navigate(s.chatgptUrl, id)
  ...
  function send(
    ...
    if (attachments?.length && convId) {
      ...
      window.wess?.sendFiles(convId, attachments)
    }
  ...
  useEffect(() => {
    ...
    for (const [convId, files] of queuedFiles.current) {
      window.wess?.sendFiles(convId, files)
    }
  ...
  function handleAgentResponse(convId: string, rawText: string): boolean {
    ...
    // ‹wess:done› is terminal and takes PRECEDENCE. If the model wrapped a final
    ...
    if (parsedToolActions.length === 0) {
      const protocolLike = /(?:^|[<{‹⟨«〈＜〈])\s*\/?\s*wess\s*:/im.test(rawText)
      const partialDone =
        /(?:^|[<{‹⟨«〈＜〈])\s*wess:done\b/im.test(rawText)
      ...
      if (protocolLike && retryCount < 2) {
        ...
        const wire =
          '{wess:result kind="protocol" status="error"}\n' +
          (partialDone
            ? 'Your final answer was truncated. Re-emit the complete answer now in one concise {wess:done} block. Do not run tools or add action tags.\n'
            : 'Your previous action response was incomplete. Re-emit that same action turn now with one complete {wess:note} and no more than 6 complete action tags. Do not repeat completed work.\n') +
          '{/wess:result}'
    ...
    if (loopCount > MAX_AGENT_LOOP) {
      ...
      const wire =
        '{wess:result kind="protocol" status="error"}\n' +
        `${limitMessage} Reply with one complete {wess:done} block and no action tags.\n` +
        '{/wess:result}'
  ...
  // combined ‹wess:result› blocks back into the thread as an invisible turn.
  async function executeTurn(convId: string, toolActions: ToolAction[], decide: (a: ToolAction) => Disposition) {
    ...
    const executeAction = async (origIdx: number) => {
      ...
      try {
        const response = await window.wess?.agentExec?.({
          cwd,
    ...
    const wire =
      ...
      '{wess:result kind="guidance" status="ok"}\n' +
  ...
  function stopSending() {
    ...
    try { window.wess?.stopGenerating?.(stopConv) } catch {}
    activeSendConv.current = null
    scheduleQueuedUserSend()
  }
  ...
  function setArchived(ids: string[], archived: boolean) {
    ...
    setConversations(prev => {
      ...
      setTimeout(() => {
        const s = snap.current
        window.wess?.saveData({ conversations: updated, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return updated
    })
  ...
  function deleteConv(ids: string[]) {
    ...
    setConversations(prev => {
      ...
      setTimeout(() => {
        const s = snap.current
        window.wess?.saveData({ conversations: rest, convOrder: s.convOrder, projects: s.projects })
      }, 0)
      return rest
    })
  ...
  function deleteAllConversations() {
    ...
    setTimeout(() => {
      window.wess?.saveData({ conversations: {}, convOrder: [], projects: snap.current.projects })
    }, 0)
  }
