// Parser for the Wess agent protocol emitted by the model.
//
// Delimiters: the canonical form is BRACES { } — {wess:run}…{/wess:run}. Braces
// aren't special in Markdown or HTML, so a bare tag survives a DOM scrape intact
// (ChatGPT's renderer parses <wess:run> into an element and the delimiters vanish),
...
// HTML, so a bare {wess:run} survives a DOM scrape intact (angle brackets don't —
// the browser parses <wess:run> into an element and the delimiters vanish).
//
...
// tag to its opener. The close allows stray whitespace (‹ / wess:run ›) because
// the model sometimes spaces it out.
const TAG_RE = new RegExp(
  `[${OPEN}]\\s*wess:(note|read|list|search|run|write|done)\\b([^${CLOSE}]*?)` +
    `(?:\\/[${CLOSE}]|[${CLOSE}]([\\s\\S]*?)[${OPEN}]\\s*\\/\\s*wess:\\1\\s*[${CLOSE}])`,
  'g',
)
...
export interface ToolResult {
  status: 'ok' | 'error'
  content: string
  /** For ‹wess:write›: a unified diff of the change, shown in the UI only (never
...
/** Serialize executed tool results into the {wess:result} blocks fed back to the model. */
...
function truncateResult(content: string, budget: number): string {
  ...
  const marker =
    `\n… [truncated by Wess — ${budget} of ${content.length} characters shown. ` +
...
export function formatAgentResults(actions: AgentAction[], results: ToolResult[]): string {
  ...
  return actions
    .map((a, i) => {
      ...
      return `{wess:result ${attrs} status="${r.status}"}\n${body}\n{/wess:result}`
    })
    .join('\n\n')
}

/** Remove wess protocol tags (any delimiter, complete/self-closing/partial/mangled) from display text. */
export function stripAgentTags(text: string): string {
  ...
  const O = OPEN
  const C = CLOSE
  // A paired ‹wess:TAG›…‹/wess:TAG›. `tag` may be a literal ("done") or an
  // alternation whose close backreferences \1 ("(note|read|…)").
  const paired = (tag: string) =>
    new RegExp(
      `[${O}]\\s*wess:${tag}\\b[^${C}]*[${C}]([\\s\\S]*?)[${O}]\\s*\\/\\s*wess:${tag.startsWith('(') ? '\\1' : tag}\\s*[${C}]`,
      'g',
    )
  return (
    text
      // ‹wess:done›…final answer…‹/wess:done› — UNWRAP: keep the answer, drop the tag.
      .replace(paired('done'), '$1')
      // ‹wess:result›… blocks are the harness's; if echoed into the answer, drop them.
      .replace(paired('result'), '')
      .replace(paired('(note|read|list|search|run|write)'), '')
      .replace(new RegExp(`[${O}]\\s*wess:(?:note|read|list|search|run|write|done)\\b[^${C}]*\\/[${C}]`, 'g'), '')
      // STREAMING: ‹wess:done› opened but hasn't closed yet — unwrap the partial
      // answer so it renders token-by-token instead of appearing all at once.
      .replace(new RegExp(`[${O}]\\s*wess:done\\b[^${C}]*[${C}]([\\s\\S]*)$`), '$1')
      ...
      .replace(
        new RegExp(
          `[${O}]\\s*wess:(?:note|read|list|search|run|write)\\b[^${C}]*[${C}][\\s\\S]*$`,
        ),
        '',
      )
      .replace(new RegExp(`[${O}]\\s*\\/?\\s*wess:[a-z]+[^${C}]*[${C}]`, 'g'), '')
      .replace(new RegExp(`[${O}]\\s*wess:[\\s\\S]*$`, 'g'), '')
      // A tag opener cut mid-token at the very end — "{w", "{we", "{/wess". A `w`
