 ...
 */
export const FOCUS_COMPOSER_EVENT = 'wess:focus-composer'
...
export default function InputBar({
  ...
  if (visibleApprovalAction) {
    ...
    return (
      <form
        className="wess-approval-frame mx-auto min-w-0"
        data-approval-state={approvalLeaving ? 'leaving' : 'entering'}
        onSubmit={(event) => event.preventDefault()}
      >
        <div className="group rounded-[22px] p-px">
          <div className="wess-composer-shell wess-approval-surface min-h-[108px] overflow-hidden rounded-[24px] border">
            <div
              ref={approvalRef}
              role="alertdialog"
              aria-label={`Approval required: ${actionLabel}`}
              tabIndex={-1}
              className="wess-approval-content flex min-h-[106px] flex-col justify-between gap-3 px-4 py-4 outline-none sm:flex-row sm:items-center sm:px-5"
              data-approval-composer
            >
              <div className="flex min-w-0 items-center gap-3">
                <span className="wess-approval-icon grid size-9 shrink-0 place-items-center rounded-xl">
                  {visibleApprovalAction.type === 'write' ? <WriteActionIcon /> : <RunActionIcon />}
                </span>
                <div className="min-w-0">
                  <div className="wess-approval-kicker mb-0.5 text-[11px] font-semibold uppercase tracking-[0.08em]">
                    Approval required
                  </div>
                  <div
                    className="wess-approval-command truncate font-mono text-[13px] font-normal"
                    title={actionLabel}
                  >
                    {actionLabel}
                  </div>
              ...
              <div className="flex shrink-0 items-center justify-end gap-2">
                <button
                  ...
                  className="wess-approval-deny h-8 rounded-lg border px-3 text-[12.5px] font-medium transition-colors"
                >
                  Deny
                </button>
                <button
                  ...
                  onClick={onApprove}
                  disabled={approvalLeaving}
                  className="wess-approval-accept h-8 rounded-lg px-3.5 text-[12.5px] font-semibold shadow-xs transition-[background-color,transform] hover:-translate-y-px"
                >
                  {approveLabel}
                </button>
              </div>
  ...
  return (
    ...
    >
      <div className="group rounded-[22px] p-px">
        <div
          className={cn(
            'wess-composer-shell chat-composer-glass rounded-[20px] border',
