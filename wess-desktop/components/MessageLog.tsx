...
import type { Conversation, AgentCall, Message } from '../hooks/useWess'
...
function HighlightedCode({ code, lang }: { code: string; lang?: string }) {
  ...
  return (
    <pre className="wess-code" data-tool-result data-tool-language={lang || 'plain'}>
...
const Markdown = memo(function Markdown({ text }: { text: string }) {
  ...
  return (
    <div className="chat-markdown wess-transcript-copy">
      <ReactMarkdown
...
function ToolCallRow({ call }: { call: AgentCall }) {
  ...
  return (
    <div className="group/tool wess-transcript-enter flex flex-col rounded-md">
      <div
        className={cn(
          'rounded-md transition-colors',
          expandable
            ? 'wess-activity-interactive cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/60'
            : 'cursor-default',
        )}
        {...(expandable
          ? {
      ...
      >
        <div
          className="wess-activity-row flex w-full select-none items-center gap-1.5 rounded-md px-0.5"
          data-activity-row="tool"
          data-tool-kind={call.kind}
        >
          <span
            className="wess-activity-secondary flex size-5 shrink-0 items-center justify-center"
            data-activity-icon
          ...
          <span
            className="wess-activity-command min-w-0 flex-1 truncate font-mono"
            title={displayLabel}
          >
            {displayLabel}
          </span>
          ...
          <span
            className="wess-activity-tertiary flex size-4 shrink-0 items-center justify-center"
      ...
      >
        {expandable && (
          <div
            className="wess-activity-rule wess-tool-result-surface mt-1 ms-[28px] cursor-default border-s ps-3"
            data-activity-rule
          ...
          >
            {hasDiff ? (
              ...
              call.kind === 'read' || (call.kind === 'run' && sourceLanguage) ? (
                ...
                <pre
                  className="wess-tool-output max-h-72 cursor-text overflow-auto py-1 pe-8 font-mono text-[11px] leading-relaxed whitespace-pre-wrap break-words select-text"
                  data-tool-result
                >
                  {call.result}
                </pre>
...
function ToolStatusDot({ status }: { status: AgentCall['status'] }) {
  if (status === 'running') {
    return (
      <span
        className="wess-tool-status-running inline-block size-3 shrink-0 animate-spin rounded-full border-2"
  ...
  if (status === 'awaiting') {
    return (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="wess-tool-status-awaiting block size-3.5 shrink-0" data-tool-status="awaiting">
  ...
  if (status === 'error' || status === 'denied') {
    return (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" className="wess-tool-status-error block size-3 shrink-0" data-tool-status={status}>
  ...
  if (status === 'blocked') {
    return (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="wess-activity-tertiary block size-3.5 shrink-0" data-tool-status="blocked">
        <circle cx="12" cy="12" r="9" /><path d="m5.6 5.6 12.8 12.8" />
      </svg>
    )
  }
  return (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" className="wess-tool-status-ok block size-3 shrink-0" data-tool-status="ok">
...
function ActivityPhaseView({ phase, active }: { phase: ActivityPhase; active: boolean }) {
  ...
  return (
    <section
      className="wess-phase-enter flex w-full flex-col"
      aria-label={label}
      data-activity-phase
    ...
    >
      <button
        ...
        className="wess-phase-trigger group/phase wess-activity-row flex w-full cursor-pointer items-center gap-1 rounded-md text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/60"
        data-activity-row="phase"
      >
        <span className={cn(
          'wess-phase-title min-w-0 truncate',
          active && 'wess-shimmer',
        )}>
          {label}
        </span>
        <span
          className="wess-activity-secondary flex size-3.5 shrink-0 items-center justify-start opacity-0 transition-opacity duration-150 group-hover/phase:opacity-100"
          data-activity-icon
        >
          <svg
      ...
      {expanded && (
        <div className="wess-activity-rule ms-2 border-s ps-2" data-activity-rule>
...
function ThinkingTag() {
  ...
  return (
    <div className="wess-thinking-enter mb-4 w-full shrink-0 px-1 py-0.5" aria-label="Thinking">
      <div
        className="wess-thinking-row wess-activity-row flex w-full items-center gap-1.5 overflow-hidden rounded-md"
        data-activity-row="thinking"
      >
        <span className="wess-thinking-label wess-thinking-pulse flex min-w-0 flex-1 items-center truncate">
          <span className="wess-shimmer">Thinking</span>
        </span>
        <span
          ref={textRef}
          className="wess-activity-tertiary flex h-6 w-[4.25rem] shrink-0 items-center justify-end tabular-nums"
        />
        <span className="flex size-4 shrink-0 items-center justify-center">
        ...
        {rows.map((row, i) => {
          if (row.kind === 'activity') {
            ...
            return (
              <div
                ...
                className="wess-transcript-enter group/message mb-2.5 flex max-w-full flex-col items-start gap-1 break-words"
              >
                <ActivityGroup
                  phases={row.phases}
                  active={active}
          ...
          return (
            <div
              key={row.key}
              className={cn(
                'wess-transcript-enter group/message flex max-w-full flex-col gap-1 break-words',
                'mb-4',
              ...
              {m.text && isUser && (
                ...
                >
                  {editing && editingMessage ? (
                    <div
                      className="wess-message-editor w-full rounded-[14px] border p-1.5"
                      data-message-editor
                    >
                      <textarea
                        autoFocus
