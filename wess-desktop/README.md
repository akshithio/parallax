...
Wess Desktop is an Electron-based coding-agent interface that connects a local project workspace to ChatGPT through the Wess browser extension.
...
Wess consists of two applications:

- `wess-desktop` — Electron and Next.js desktop interface
- `wess-extension` — Chrome extension that bridges ChatGPT to the desktop app
...
5. The desktop app parses any `‹wess:...›` tool actions.
...
- pnpm 10
- Google Chrome
- The Wess Chrome extension installed and enabled
...
4. Choose the `wess-extension` directory.
5. Open ChatGPT in Chrome.
6. Confirm that the Wess extension is connected.
...
`latest-mac.yml` metadata that installed copies of Wess use to discover updates.

The updater:

- checks shortly after an installed build starts and every four hours afterward;
- downloads a newer release in the background;
- installs the downloaded release when Wess quits; and
...
├── hooks/
│   └── useWess.ts       Main conversation and agent-loop state
├── lib/
...
On the first message of a conversation, Wess prepends an agent-system prompt to the message sent to ChatGPT.
...
```text
‹wess:read path="src/index.ts" /›
```

```text
‹wess:search query="TODO|FIXME" path="src" /›
```

```text
‹wess:run›pnpm test‹/wess:run›
```

```text
‹wess:write path="src/example.ts"›
