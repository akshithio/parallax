...
//  2. Relays stream diagnostics while a Parallax-authored turn is active.
...
(function () {
  ...
  // note) could break a conversation Parallax wasn't even involved in.
  let armed = false;
  let activeSink = null;
  ...
  function postMetric(metric) {
    try {
      window.postMessage({ __parallax_net_metric: true, metric }, '*');
    } catch {}
  }
  ...
  try {
    ...
    if (OrigES && !window.__parallaxESHooked) {
      window.__parallaxESHooked = true;
  ...
  function feedWsPayload(sink, raw, sourceState) {
    ...
    const decoder = globalThis.ParallaxProtocolCore?.streamEvents;
  ...
  try {
    ...
    if (OrigWS && !window.__parallaxWSHooked) {
      window.__parallaxWSHooked = true;
  ...
  // One parser per Parallax turn. Fetch, EventSource, and WebSocket frames all feed the
