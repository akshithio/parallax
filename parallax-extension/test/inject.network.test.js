...
test('a fetch handoff continues on the socket and completes from network text', () => {
  ...
  const responsePosts = harness.messages.filter((message) => message.__parallax_net)
...
test('arming the page hook acknowledges the exact turn before submission', () => {
  ...
  harness.listeners.get('message')({
    ...
    data: { __parallax_arm: true, armed: true, turnId: 'turn-ready' },
  })

  const ready = harness.messages.find((message) => message.__parallax_net_arm_ready)
  assert.equal(ready.__parallax_net_arm_ready, true)
...
test('a message boundary cannot truncate a turn that continues streaming', () => {
  ...
  assert.equal(sink.done, true)
  const response = harness.messages.filter((message) => message.__parallax_net).at(-1)
  assert.equal(
    response.text,
