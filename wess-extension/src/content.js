...
globalThis.__wessContentLoaded = true;

let backgroundPort = null;
let sending = false;
// Start inert. The background explicitly resumes only tabs that belong to a Wess
...
window.addEventListener('message', (e) => {
  ...
  if (d.__wess_models === true && Array.isArray(d.models)) {
    postToBg({ type: 'models', models: d.models });
    return;
  }
  if (d.__wess_net_request === true) {
  ...
  if (d.__wess_net_arm_ready === true) {
    if (
      !currentSend ||
      !d.armed ||
  ...
  if (d.__wess_net_metric === true && d.metric) {
    postToBg({
  ...
  if (d.__wess_net_debug === true) { wlog(`sse sample: ${d.sample}`); return; }
  if (d.__wess_net !== true) return;
  if (!currentSend) {
    console.warn('[Wess] net response dropped — no currentSend (was port reset?)');
    return;
  }
...
function connectBackground() {
  ...
  try {
    ...
    console.warn('[Wess] extension context lost — this page needs a refresh');
    showReloadWarning();
    return;
  }

  try {
    backgroundPort = chrome.runtime.connect({ name: 'wess-content' });
  } catch (e) {
    reconnectAttempts++;
    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
      console.warn('[Wess] max reconnect attempts reached — extension may need a page refresh');
      showReloadWarning();
      return;
    }
    const delay = Math.min(1000 * Math.pow(2, reconnectAttempts), 30000);
    console.warn('[Wess] connect failed (attempt', reconnectAttempts, '):', e.message, '— retry in', delay, 'ms');
  ...
  backgroundPort.onDisconnect.addListener(() => {
    ...
    if (chrome.runtime.lastError) {
      console.warn('[Wess] port disconnected:', chrome.runtime.lastError.message);
    }
    sending = false;
  ...
  for (const msg of queue) {
    try { backgroundPort.postMessage(msg); } catch (e) {
      console.error('[Wess] flush failed:', e);
    }
  }
...
function requestModelsFromPage() {
  const ask = () => {
    if (standby) return;
    try { window.postMessage({ __wess_request_models: true }, '*'); } catch (_) {}
  };
  ask();
...
function handleMessage(msg) {
  ...
  // This tab isn't the one Wess drives — go quiet so we don't report our URL or
  // act on anything meant for the owned tab.
  if (msg.type === 'standby') {
    standby = true;
    stopOwnedPageFeatures();
    console.log('[Wess] standing down — this is not the Wess tab');
    return;
  }
  ...
  if (msg.type === 'resume') {
    if (standby) {
      ...
      console.log('[Wess] resumed — this tab is Wess-owned');
    }
    return;
  }
  if (standby) {
    ...
    startOwnedPageFeatures();
    console.log('[Wess] resumed from an owned page command');
  }
  switch (msg.type) {
    case 'recover_turn':
    ...
    case 'switch_model':
      ...
      console.log('[Wess] switch_model recv → model:', msg.model || '(keep)', '| intelligence:', msg.intelligence || '(keep)', sending ? '(BUSY)' : '');
      if (sending) {
...
function scrapeModelMenu(force = false) {
  try {
    ...
    console.log('[Wess] model menu →', currentModel || '(?)',
...
async function seedModelMenu() {
  ...
  try {
    ...
    for (let i = 0; i < 40; i++) {
      ...
      if (lastMenuSig) {
        seededMenu = true;
        console.log('[Wess] seed skipped — menu already captured');
        return;
      }
    ...
    if (!trigger) { console.warn('[Wess] seed: model button not found after 20s'); return; }
    if (standby) return;
    seededMenu = true;
    console.log('[Wess] seeding model menu via', (trigger.textContent || '').trim().slice(0, 24));
    ...
    if (modelRow) {
      ...
      console.warn('[Wess] seed: model row not found (menu may not have opened)');
    }
    document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
    await sleep(150);
    document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  } catch (e) {
    console.warn('[Wess] seed failed:', e);
  } finally {
    seedingMenu = false;
  }
}
...
// menu state before Wess is allowed to submit a message.
...
async function selectModelViaDom(model, intelligence) {
  ...
  try {
    ...
    console.log('[Wess] switch → model:', model || '(keep)', '| intelligence:', intelligence || '(keep)');
    ...
    if (model) {
      ...
      if (!currentModel || normModelText(currentModel) !== normModelText(model)) {
        ...
        realClick(target);
        console.log('[Wess] switch: clicked model', model);
    ...
    if (intelligence) {
      ...
      if (el) {
        ...
        console.log('[Wess] switch: confirmed intelligence', intelligence);
      } else {
        const state = scrapeModelMenu(true);
    ...
    console.warn('[Wess] selectModelViaDom failed:', e);
...
function conversationId(url) {
  return globalThis.WessProtocolCore?.conversationId(url) || null;
}
...
// while Wess is driving a turn in this tab.
function armNetHooks(on, turnId) {
  if (!on) {
    try {
      window.postMessage({ __wess_arm: true, armed: false, turnId: turnId || '' }, '*');
    } catch (_) {}
  ...
  try {
    window.postMessage({ __wess_arm: true, armed: true, turnId: turnId || '' }, '*');
  } catch (_) {
...
async function sendMessageToChatGPT(text, msgId, model, intelligence, expectUrl) {
  ...
  try {
    ...
    console.log('[Wess] waiting for network response');
...
function isCompleteWessResponse(value) {
  const text = String(value || '').trim();
  const opening = /\{wess:(note|run|write|patch|done)\b[^}]*\}/g;
  let cursor = 0;
  let found = false;
  let match = null;
  while ((match = opening.exec(text)) !== null) {
    if (text.slice(cursor, match.index).trim()) return false;
    const close = `{/wess:${match[1]}}`;
...
function completedResponseAfterPrompt(prompt) {
  ...
  for (let i = userIndex + 1; i < messages.length; i++) {
    ...
    if (isCompleteWessResponse(text)) return text;
  }
  return '';
}
...
function postToBg(msg) {
  if (backgroundPort) {
    try {
      ...
      console.error('[Wess] postToBg failed:', e);
...
function showReloadWarning() {
  const existing = document.getElementById('__wess_reload_warning');
  if (existing) return;
  const banner = document.createElement('div');
  banner.id = '__wess_reload_warning';
  ...
  banner.textContent = '⚠ Wess extension was reloaded — click to refresh this page';
  banner.onclick = () => location.reload();
  document.body.prepend(banner);
  // Also try to auto-reload after 30s
  setTimeout(() => { if (document.getElementById('__wess_reload_warning')) location.reload(); }, 30000);
}
