...
//  2. Relays stream diagnostics while a Wess-authored turn is active.
...
(function () {
  if (window.__wessNetHooked) return;
  window.__wessNetHooked = true;

  // ARMED ONLY DURING A WESS TURN. Everything below — teeing response streams,
  ...
  // note) could break a conversation Wess wasn't even involved in.
  let armed = false;
  let activeSink = null;
  ...
  function post(turnId, text, done, completion) {
    try {
      if (done) console.log('[Wess] net response complete, chars:', String(text || '').length);
      window.postMessage({
        __wess_net: true,
        source: 'network',
        turnId,
  ...
  function postMetric(metric) {
    try {
      window.postMessage({ __wess_net_metric: true, metric }, '*');
    } catch {}
  }
  ...
  function postDebug(msg) {
    try {
      console.log('[Wess][net]', msg);
      window.postMessage({ __wess_net_debug: true, sample: msg }, '*');
    } catch {}
  }
  ...
  try {
    ...
    if (OrigES && !window.__wessESHooked) {
      window.__wessESHooked = true;
  ...
  function feedWsPayload(sink, raw, sourceState) {
    ...
    const decoder = globalThis.WessProtocolCore?.streamEvents;
  ...
  try {
    ...
    if (OrigWS && !window.__wessWSHooked) {
      window.__wessWSHooked = true;
  ...
  // One parser per Wess turn. Fetch, EventSource, and WebSocket frames all feed the
