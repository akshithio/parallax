...
function loadContent() {
  ...
  return {
    ...
    helpers: {
      ...
      isCompleteWessResponse: context.isCompleteWessResponse,
    },
    FakeTextArea,
    setComposer(element) {
...
test('page recovery accepts only complete Wess response envelopes', () => {
  const harness = loadContent()
  const complete = [
    '{wess:note}Inspecting data{/wess:note}',
    '{wess:run}find . -maxdepth 2 -type f -print{/wess:run}',
  ].join('\n')

  assert.equal(harness.helpers.isCompleteWessResponse(complete), true)
  assert.equal(
    harness.helpers.isCompleteWessResponse(
      '{wess:note}Inspecting data{/wess:note}\n{wess:run}find .',
    ),
    false,
  )
  assert.equal(harness.helpers.isCompleteWessResponse('ordinary rendered text'), false)
})

test('a rejected internal follow-up is removed without erasing a different draft', () => {
  const harness = loadContent()
  const internal = '{wess:result kind="run"}output{/wess:result}'
